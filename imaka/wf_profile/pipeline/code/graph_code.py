...
###################################
###################################
def gif_1_mat_static(mat, tmax,  title = None, label_1 = None, out_file='out.gif', std_val = 3):
    fig, (ax, cax) = plt.subplots(ncols=2,figsize=(6,6), gridspec_kw={"width_ratios":[1,0.05]})
    fig.subplots_adjust(wspace=0.3)
    fig.suptitle(str(title))
    
    mean, median, std = sigma_clipped_stats(mat, sigma=3)
    max_val = mean + std_val*std
    min_val = mean - std_val*std
    #max_val = np.amax(mat)
    #min_val = np.amin(mat)
...
# Triangular matricies with a color bar.
###################################
###################################
def gif_ccor_mat_scale(mat_arr, tmax, title=None, out_file='out.gif'):
    #min_val = np.min(mat_arr, axis=(0, 1, 3, 4))
    #max_val = np.max(mat_arr, axis=(0, 1, 3, 4))
    
    mean, __, std = sigma_clipped_stats(mat, sigma=5)
    max_val = mean + std_val*std
    min_val = mean - std_val*std
    ...
    for t in range(tmax):
        for i in range(n_len):
            for j in range(n_width):
                if i>=j:
                    ax = axes[i][j]
                    im = ax.matshow(mat_arr[i][j][t], vmin = min_val, vmax=max_val)
                    #im = ax.matshow(mat_arr[i][j][t])
                    ax.tick_params(axis='x', which='both', bottom=False, top=False, labelbottom=False)
                    ax.tick_params(axis='y', which='both', right=False, left=False, labelleft=False)
                if i==0 and j==0:
