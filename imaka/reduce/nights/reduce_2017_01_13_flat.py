...
###obj_o613.fits in log but not in data.... also 624
    ...
    img_files = ['obj_o{0:03d}.fits'.format(ii) for ii in fnum]
    reduce_fli.flat_sky_reduction(img_file, sky_frame=sky_dir + 'pleiades_sky_598.fits', flat_frame=flat_dir + 'flat.fits')
...
def find_stars_pleiades_binned_closed():
    data_dir = '/Volumes/g/lu/data/imaka/2017_01_13/fli/reduce/'
    os.chdir(data_dir)
...
#     fnum = fnum1 + fnum2 
#     img_files = ['obj_o{0:03d}_bin_nobkg.fits'.format(ii) for ii in fnum]
#     reduce_fli.calc_star_stats(img_files, output_stats='stats_open1.fits')
